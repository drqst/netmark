...
# Services), but the persistent volume claim the volume container owns and
...
if ! kubectl cluster-info >/dev/null 2>&1; then
  echo "No Kubernetes cluster is reachable - nothing to stop."
  exit 0
fi
...
kubectl -n "$NAMESPACE" wait --for=delete pod --all --timeout=180s 2>/dev/null || true

remaining=$(kubectl -n "$NAMESPACE" get pods --no-headers 2>/dev/null | wc -l | tr -d ' ')
if [ "$remaining" -ne 0 ]; then
  echo "warning: $remaining pod(s) still terminating in namespace $NAMESPACE" >&2
fi
